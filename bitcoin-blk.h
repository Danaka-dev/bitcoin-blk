...
bool keyAddressToP2PKH( const std::string &address ,ByteVector &pubkey ) {
    ByteVector decode;
...
uint256 getHashFromBin( const ByteVector &bin ) {
    uint256 hash1 ,hash2;
...
uint256 getHashFromHex( const std::string &hex ) {
...
template <>
ByteStream &ByteStream::operator <<( const uint256 &v ) {
    memcpy( bin.data() + pos ,v.data ,32 ); pos += 32; return *this;
}

template <>
ByteStream &ByteStream::operator <<( const MinEnc &v ) {
    if( v.value < 0xff ) {
...
Script getScriptCoinbaseIn_BIP0034( uint32_t height ) {
    Script s;
...
Script getScriptForAddress( const std::string &address ) {
    ByteVector p2pkh;
...
template <>
ByteStream &ByteStream::operator <<( const Script &v ) {
...
TxIn getTxInCoinbase( uint32_t blockHeight ) {
    TxIn tx;
...
template <>
ByteStream &ByteStream::operator <<( const TxIn &v ) {
...
TxOut getTxOutForAddress( const std::string &address ,int64_t amount ) {
    TxOut tx;
...
TxOut getTxOutWithScript( const std::string &script ,int64_t amount ) {
    TxOut tx;
...
template <>
ByteStream &ByteStream::operator <<( const TxOut &v ) {
...
template <>
ByteStream &ByteStream::operator <<( const Transaction &tx ) {
    *this << tx.version;
...
std::string Transaction::toHex( const std::string &anExtraPayload ) const {
    ByteVector cb;
